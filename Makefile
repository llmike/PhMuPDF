...
$(OUT)/platform/qnx/%.o : platform/qnx/%.c | $(ALL_DIR)
	$(CC_CMD) $(PHOTON_CFLAGS) $(CURL_CFLAGS)

$(OUT)/platform/x11/curl/%.o : platform/x11/%.c | $(ALL_DIR)
	mkdir -p $(OUT)/platform/x11/curl
	$(CC_CMD) $(X11_CFLAGS) $(CURL_CFLAGS) -DHAVE_CURL

$(OUT)/platform/qnx/curl/%.o : platform/qnx/%.c | $(ALL_DIR)
	mkdir -p $(OUT)/platform/qnx/curl
	$(CC_CMD) $(X11_CFLAGS) $(CURL_CFLAGS) -DHAVE_CURL
...
ifeq "$(NOPHOTON)" ""
MUVIEW_PHOTON := $(OUT)/phmupdf
$(MUVIEW_PHOTON) : $(MUPDF_LIB) $(MUPDF_JS_NONE_LIB) $(THIRD_LIBS)
$(MUVIEW_PHOTON) : $(addprefix $(OUT)/platform/qnx/, phcallbacks.o phmupdf.o phpdf.o phpictures.o)
	$(LINK_CMD) $(PHOTON_LIBS)
	@phabbind $(MUVIEW_PHOTON) ./platform/qnx/Icon.wgti
endif
...
MUVIEW := $(MUVIEW_X11) $(MUVIEW_PHOTON)
MUVIEW_V8 := $(MUVIEW_X11_V8) $(MUVIEW_PHOTON_V8)
MUVIEW_CURL := $(MUVIEW_X11_CURL) $(MUVIEW_PHOTON_CURL)
